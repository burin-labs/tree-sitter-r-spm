# Contributing to `tree-sitter-r-spm`

## Scope

This repository is a packaging wrapper, not a product. It exists so Swift
projects can depend on the upstream R tree-sitter grammar without running
...
[`tree-sitter-r`](https://github.com/r-lib/tree-sitter-r) tree-sitter grammar. The generated parser
(`Sources/TreeSitterR/src/parser.c` and any `scanner.c`) is **vendored** so
...
You should see `T _tree_sitter_r`. If the grammar ships an
